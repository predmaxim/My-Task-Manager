...
export const getTasks: RequestHandler = async (req: Request, res: Response) => {
  try {
    ...
    const tasks = await prisma.task.findMany({where: {projectId: parseInt(id)}, orderBy: {index: 'asc'}});
...
export const deleteTask: RequestHandler = async (req: Request, res: Response) => {
  try {
    const id = z.object({id: z.string()}).parse(req.query).id;
    const removedTask = await prisma.task.delete({where: {id: parseInt(id)}});
...
export const updateTask: RequestHandler = async (req: Request, res: Response) => {
  try {
    ...
    const updatedTask = await prisma.task.update({where: {id: parseInt(id)}, data: task});
