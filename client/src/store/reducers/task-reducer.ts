...
export const taskReducer = (
  ...
  action:
    ...
    | TaskActionType<typeof DELETE_TASK, TaskType['id']>
  ...
  switch (action.type) {
    ...
    case UPDATE_TASK:
      ...
      const updatedTasksList = state.tasks.map((task) =>
        task.id === updatedTask.id || (task.number === updatedTask.number && task.project === updatedTask.project)
          ? {...task, ...updatedTask}
          : task
      );
      return {
    ...
    case DELETE_TASK:
      ...
      const tasksWithoutDeleted = state.tasks.filter((task) => task.id !== taskIdToDelete);
      return {
        ...state,
...
export const setTasksAction = (payload: TaskType[]): TaskActionType<typeof SET_TASKS, TaskType[]> => ({
  type: SET_TASKS,
  payload
});

export const createTaskAction = (payload: TaskType): TaskActionType<typeof CREATE_TASK, TaskType> => ({
  type: CREATE_TASK,
  payload
});

export const updateTaskAction = (payload: TaskType): TaskActionType<typeof UPDATE_TASK, TaskType> => ({
  type: UPDATE_TASK,
  payload
});

export const deleteTaskAction = (payload: TaskType['id']): TaskActionType<typeof DELETE_TASK, TaskType['id']> => ({
  type: DELETE_TASK,
  payload
});

export const setLoadingTasksAction = (payload: boolean): TaskActionType<typeof SET_LOADING, boolean> => ({
  type: SET_LOADING,
  payload
});
