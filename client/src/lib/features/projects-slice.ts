...
export const projectsSlice = createSlice({
  ...
  reducers: {
    ...
    updateProject: (state, action: PayloadAction<PartialProjectType>) => {
      ...
      const index = state.projects.findIndex((project) => project.id === action.payload._id);
      state.projects[index] = {...state.projects[index], ...action.payload};
    },
    deleteProject: (state, action: PayloadAction<ProjectType['_id']>) => {
      if(!state.projects?.length) {
        console.log('No projects found');
        return;
      }
...
export const {setCurrentProject, deleteProject, createProject,setProjects, updateProject, setLoading} = projectsSlice.actions;
